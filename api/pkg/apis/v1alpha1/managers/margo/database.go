...
type DeploymentBundleRow struct {
	DeviceClientId string
	Manifest       margoStdAPI.UnsignedStateManifest
	ArchivePath    string
	// update time
	UpdatedAt time.Time
}
...
func (db *MargoDatabase) UpsertDeploymentCurrentState(ctx context.Context, deploymentId string, current sbi.AppDeployment, publishEvent bool) error {
	// Get existing deployment or create new one if it doesn't exist
	deployment, err := db.GetDeployment(ctx, deploymentId)
	if err != nil {
		// If deployment doesn't exist, create a new DeploymentDatabaseRow
...
func (db *MargoDatabase) UpsertDeploymentBundle(ctx context.Context, bundleRow DeploymentBundleRow, publishEvent bool) error {
	...
	if err != nil {
		db.MgrContext.Logger.ErrorfCtx(ctx, "UpsertDeploymentBundle: Failed to update deployment bundle cache'%s': %v", deviceClientId, err)
	...
	db.MgrContext.Logger.InfofCtx(ctx, "UpsertDeploymentBundle: deployment bundle '%s' stored successfully", deviceClientId)
	return nil
}
...
func (db *MargoDatabase) DeleteDeploymentBundle(ctx context.Context, deviceClientId string, publishEvent bool) error {
	...
	if err != nil {
		db.MgrContext.Logger.ErrorfCtx(ctx, "DeleteDeploymentBundle: Failed to delete deployment bundle for device client: %s, %v", deviceClientId, err)
		return fmt.Errorf("failed to delete deployment bundle for device client: %s, %s", deviceClientId, err)
	}

	db.MgrContext.Logger.InfofCtx(ctx, "DeleteDeploymentBundle: deployment bundle for device client: '%s' deleted successfully", deviceClientId)
